...
export const groupArrayByKey = ({data, key, values, template, sorter  }:{data: any[], key: string, values: string[],template:any, sorter: Function}) => {
    ...
    for (const key in list) {
        if (list.hasOwnProperty(key)) {
            ...
            series.push({ ...template,...list[key]})
        }
    }
    
    return series.sort(sorter|| 0);
}
